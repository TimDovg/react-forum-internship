...
const CreatePost = ({ history }) => {
  ...
  const onTitleHandler = ({ target: { value } }) =>
    setState({ ...state, title: value })

  const onBodyHandler = ({ target: { value } }) =>
  ...
  return (
    <>
      <div className="m-4-5">
        ...
        />
        <label className="mt-4" htmlFor="forTitle">
        ...
        <div className="d-flex justify-content-end">
          {!state.valid ? (
            <small className="text-danger mr-auto mt-2">
              Заполните все поля!
            </small>
          ) : state.created ? (

...
export const Albums = ({ match }) => {
  ...
  const onSearchHandler = ({ target: { value } }) => {
    ...
    const filterAlbums = state.albums.filter(album =>
      re.test(UserIdUserName[album.userId]),
    )
  ...
  return (
    <>
      <label htmlFor="forSearch" className="font-weight-bold ml-4-5 mt-3">
        Поиск:{' '}
      </label>
      <input
      ...
      />
      <small className="d-block ml-4-5 mt-2">
        Найдено альбомов: <strong>{state.filterAlbums.length}</strong>
      </small>
              ...
              <div className="card bg-dark text-white">
                <div className="card-body d-flex flex-column">
                  ...
                  <p className="font-italic text-secondary">
                    Автор: {UserIdUserName[album.userId]}
                  </p>
                  <Link
                    to={`${match.url}/${album.id}`}

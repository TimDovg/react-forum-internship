...
export const ShowPost = ({ match }) => {
  ...
  const { state: statePost, getPostById, addMyComment } = useContext(
    PostContext,
  )
  const {
    state: { UserIdUserName },
    getUsers,
  ...
  const typeCommentHandler = () =>
    setValue({ ...internalState, typeComment: true })

  // для практики деструкторизации
  const nameHandler = ({ target: { value } }) =>
    setValue({ ...internalState, name: value })

  const emailHandler = ({ target: { value } }) =>
    setValue({ ...internalState, email: value })

  const bodyHandler = ({ target: { value } }) =>
    setValue({ ...internalState, body: value })

  const submitHandler = () => {
    if (
      !internalState.name.trim() ||
      !internalState.email.trim() ||
      !internalState.body.trim()
    ) {
      ...
      {statePost.loading ? (
        ...
        <>
          ...
          <h4 className="ml-4 mt-4">
            Комментарии ({statePost.comments && statePost.comments.length})
          </h4>
          <CommentForm
